...
+221 78 295 37 80
    ...
    <a class="nav-account" id="navAccount" href="/connexion" title="Connexion à votre espace" aria-label="Connexion à votre espace">
  ...
  // La feuille de style de l'accès au compte est injectée ici,
  // pour ne pas avoir à modifier les cinq pages HTML.
  if (!document.getElementById("compteCss")) {
  ...
  // ---------- Accès au compte ----------
  // Le site est servi en statique : on demande au serveur qui est
  // connecté, et on ajuste le bouton. Sans réponse, il reste
  // « Connexion » — le pire cas est un clic de plus, jamais une erreur.
  (async () => {
    const link = document.getElementById("navAccount");
    if (!link) return;
    try {
      const res = await fetch("/api/moi", { credentials: "same-origin" });
      if (!res.ok) return;
      const me = await res.json();
      if (!me.signedIn) return;
      link.href = "/espace";
      link.querySelector(".nav-account-label").textContent = "Mon espace";
      link.title = "Votre espace client";
      link.setAttribute("aria-label", "Votre espace client");
      if (me.staff) {
        const admin = document.createElement("a");
        admin.className = "nav-account is-ghost";
        admin.href = "/admin";
        admin.title = "Administration";
        admin.setAttribute("aria-label", "Administration");
        admin.innerHTML = `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.4"/><path d="M5 20a7 7 0 0 1 14 0"/></svg><span class="nav-account-label">Administration</span>`;
        link.parentNode.insertBefore(admin, link);
      }
    } catch (e) { /* hors ligne : on garde « Connexion » */ }
  })();
